...
def test_logout_fail_twice(url):
    ...
    assert requests.post(url + "auth/logout", json={"token": user1["token"]}).json()["is_success"] == False

def test_logout_fail_non_exist_user():
    requests.delete(url + "clear")
    token = "non exist"
    assert requests.post(url + "auth/logout", json={"token": token}).json()["is_success"] == False


def test_logout_success():
    requests.delete(url + "clear")
    token = register_new_account(url)["token"]
    assert requests.post(url + "auth/logout", json={"token": token}).json()["is_success"] == True
