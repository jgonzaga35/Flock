...
def test_react_succesfully():
    ...
    public_channel_id = channels_create(user_a['token'], "public_channel", True)['channel_id']
    # user_b join the channel created by user_a
    channel_join(user_b['token'], public_channel_id)
    message_id = message_send(user_a['token'], public_channel_id, "Nice to see you mate")['message_id']
...
def test_react_invalid_message_id_in_channel():
    ...
    public_channel_id_a = channels_create(user_a['token'], "public_channel_a", True)['channel_id']
    #user_b create a channel and send message in his own channel
    public_channel_id_b = channels_create(user_b['token'], "public_channel_b", True)['channel_id']
    message_id_b = message_send(user_b['token'], public_channel_id_b, "I am in channel_b")
