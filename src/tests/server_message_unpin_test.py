...
# Fixture which creates a sample public channel
...
def test_message_unpin_public_simple(create_channel):
...
def test_message_unpin_private_simple(url):
    requests.delete(url + "clear")
    user = http_register_n_users(url, 1)
...
def test_message_unpin_invalid_message_id(create_channel):
    url, user, channel, message = create_channel
    ...
    # Attempt to unpin with invalid message_id
    response = requests.post(
        url + "message/unpin",
...
def test_message_already_unpinned(create_channel):
    url, user, channel, message = create_channel

    # Pin message, ensuring it gets pinned
    response = requests.post(
        url + "message/pin",
        json={"token": user["token"], "message_id": message["message_id"]},
    )
    ...
    # Unpin message, ensuring unpinned
    response = requests.post(
        url + "message/unpin",
        json={"token": user["token"], "message_id": message["message_id"]},
    )
    ...
    # Attempt to unpin a message that is already pinned
    response = requests.post(
        url + "message/unpin",
        json={"token": user["token"], "message_id": message["message_id"]},
    )
