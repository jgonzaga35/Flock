...
def test_sendlater_single():
    ...
    send_at = time.time() + 0.5 * DELAY_SCALE
    ...
    assert time.time() < send_at, ERR_TOO_SLOW
    assert len(messages["messages"]) == 0

    # wait for the message to be sent
    time.sleep(send_at - time.time() + 0.5 * DELAY_SCALE)
    ...
    assert messages["messages"][0]["time_created"] - send_at < 0.5
...
def test_sendlater_backwards():
    ...
    send_at_a = time.time() + 1 * DELAY_SCALE
    # message b will be sent *before* message a
    send_at_b = send_at_a - 0.5 * DELAY_SCALE
    ...
    assert time.time() < send_at_b, ERR_TOO_SLOW
    assert len(messages["messages"]) == 0

    # wait for the first message (message B) to be sent
    time.sleep(send_at_b - time.time() + 0.25 * DELAY_SCALE)

    messages = channel_messages(usera["token"], channel_id, start=0)
    assert time.time() < send_at_a, ERR_TOO_SLOW
    assert len(messages["messages"]) == 1
    assert messages["messages"][0]["message"] == "message b"
    assert (messages["messages"][0]["time_created"] - send_at_b) < 0.5

    # wait for the second message (message A) to be sent
    time.sleep(send_at_a - time.time() + 0.25 * DELAY_SCALE)

    messages = channel_messages(userb["token"], channel_id, start=0)
    assert len(messages["messages"]) == 2
    assert messages["messages"][0]["message"] == "message a"
    assert (messages["messages"][0]["time_created"] - send_at_a) < 0.5
